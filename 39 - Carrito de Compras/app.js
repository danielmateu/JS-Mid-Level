...
function renderShoppingCart(){
    ...
    document.querySelectorAll('.addOne').forEach(button => {
        button.addEventListener('click',e =>{
            const id = parseInt(button.getAttribute('data-id'), 10);shoppingCart.methods.add(id,1);
            renderShoppingCart();
        });
    });
    ...
    document.querySelectorAll('.removeOne').forEach(button => {
        button.addEventListener('click',e =>{
            const id = parseInt(button.getAttribute('data-id'), 10);shoppingCart.methods.remove(id,1);
            renderShoppingCart();
        })
    });
